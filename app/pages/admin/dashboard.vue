...
const quickActions = [
    {
        ...
        to: '/admin/pending',
        color: 'amber',
        count: pendingChannelsData.value.total
    },
    {
        label: 'จัดการผู้ใช้',
...
<template>
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <div class="px-2.5 py-1 bg-indigo-100 dark:bg-indigo-900/30 rounded-full">
                            <span class="text-xs font-bold text-indigo-700 dark:text-indigo-400">ADMIN</span>
                        </div>
                    </div>
        ...
        <!-- <div
            ...
            <div class="flex items-center gap-3 mb-6">
                <div class="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                    <UIcon name="i-lucide-zap" class="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                </div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">การดำเนินการด่วน</h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <NuxtLink v-for="action in quickActions" :key="action.label" :to="action.to"
                    class="group relative overflow-hidden bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-200 dark:border-gray-600 hover:border-indigo-500 dark:hover:border-indigo-500 transition-all hover:shadow-lg">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p v-if="action.count !== undefined" class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ action.count }} รายการ
                                </p>
                            </div>
                        </div>
    ...
    <!-- Loading Indicator -->
    <div v-if="loading" class="fixed bottom-6 right-6 z-50">
        <div
            class="flex items-center gap-3 px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl">
            <UIcon name="i-lucide-loader-circle" class="w-5 h-5 animate-spin text-indigo-600" />
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">กำลังอัปเดตข้อมูล...</span>
        </div>
    </div>
